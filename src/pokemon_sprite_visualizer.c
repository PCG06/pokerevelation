...
static void LoadBattleBg(u8 battleBgType, enum BattleEnvironments battleEnvironment)
{
    ...
    case MAP_BATTLE_SCENE_KYOGRE:
        DecompressDataWithHeaderVram(gBattleEnvironmentTiles_Underwater, (void*)(BG_CHAR_ADDR(2)));
        DecompressDataWithHeaderVram(gBattleEnvironmentTilemap_Underwater, (void*)(BG_SCREEN_ADDR(26)));
        LoadPalette(gBattleEnvironmentPalette_Underwater, 0x20, 0x60);
        break;
...
static void UpdateBattleBg(u8 taskId, bool8 increment)
{
    ...
    if (data->battleBgType == MAP_BATTLE_SCENE_NORMAL)
    {
        if (increment)
        {
            if (data->battleEnvironment == BATTLE_ENVIRONMENT_ICE)
                data->battleBgType += 1;
            else
    ...
    else if (data->battleBgType == MAP_BATTLE_SCENE_GYM)
    {
        ...
        else
        {
            ...
            data->battleEnvironment = BATTLE_ENVIRONMENT_ICE;
        }
    }
