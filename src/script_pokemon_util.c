...
static u32 GiveMonSet(u16 species, u8 otIdType)
{
    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, MAX_LEVEL,
        ...
        (u8 *)gPokemonSets[species].evs, (u8 *)gPokemonSets[species].ivs, (u16 *)gPokemonSets[species].moves, SHINY_MODE_RANDOM, FALSE, (u8)gPokemonSets[species].teraType, 0, otIdType, 0);
}
...
void ScrCmd_createmon(struct ScriptContext *ctx)
{
    ...
    gSpecialVar_Result = ScriptGiveMonParameterized(side, slot, species, level, item, ball, nature, abilityNum, gender, evs, ivs, moves, shinyMode, gmaxFactor, teraType, dmaxLevel, OT_ID_PLAYER_ID, 0);
}
