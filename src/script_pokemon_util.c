...
void CreateScriptedWildMon(u16 species, u8 level, u16 item)
{
    ...
    if (OW_SYNCHRONIZE_NATURE > GEN_3)
        CreateMonWithNature(&gEnemyParty[0], species, level, USE_RANDOM_IVS, PickWildMonNature(), OT_ID_PLAYER_ID, 0);
    else
...
void CreateScriptedDoubleWildMon(u16 species1, u8 level1, u16 item1, u16 species2, u8 level2, u16 item2)
{
    ...
    if (OW_SYNCHRONIZE_NATURE > GEN_3)
        CreateMonWithNature(&gEnemyParty[0], species1, level1, 32, PickWildMonNature(), OT_ID_PLAYER_ID, 0);
    else
    ...
    if (OW_SYNCHRONIZE_NATURE > GEN_3)
        CreateMonWithNature(&gEnemyParty[1], species2, level2, 32, PickWildMonNature(), OT_ID_PLAYER_ID, 0);
    else
 ...
 */
static u32 ScriptGiveMonParameterized(u8 side, u8 slot, u16 species, u8 level, u16 item, enum PokeBall ball, u8 nature, u8 abilityNum, u8 gender, u8 *evs, u8 *ivs, u16 *moves, bool8 isShiny, bool8 gmaxFactor, u8 teraType, u8 dmaxLevel, u8 otIdType, u32 fixedOtId)
{
    u16 nationalDexNum;
    int sentToPc;
    struct Pokemon mon;
    ...
    if ((gender == MON_MALE && genderRatio != MON_FEMALE && genderRatio != MON_GENDERLESS)
     ...
     || (gender == MON_GENDERLESS && genderRatio == MON_GENDERLESS))
        CreateMonWithGenderNatureLetter(&mon, species, level, 32, gender, nature, 0, otIdType, fixedOtId);
    else
        CreateMonWithNature(&mon, species, level, 32, nature, otIdType, fixedOtId);
...
u32 ScriptGiveMon(u16 species, u8 level, u16 item)
{
    ...
    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, level, item, ITEM_POKE_BALL, NUM_NATURES, NUM_ABILITY_PERSONALITY, MON_GENDERLESS, evs, ivs, moves, FALSE, FALSE, NUMBER_OF_MON_TYPES, 0, OT_ID_PLAYER_ID, 0);
}

u32 BirchCase_GiveMonParameterized(u16 species, u8 level, u16 item, u8 ball, u8 nature, u8 abilityNum, u8 gender, u8 *evs, u8 *ivs, u16 *moves, bool8 isShiny, bool8 gmaxFactor, u8 teraType, u8 dmaxLevel)
{
    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, level, item, ball, nature, abilityNum, gender, evs, ivs, moves, isShiny, gmaxFactor, teraType, dmaxLevel, OT_ID_PLAYER_ID, 0);
}

static u32 GiveMonSet(u16 species, u8 otIdType)
{
    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, MAX_LEVEL,
        ...
        (u8 *)gPokemonSets[species].evs, (u8 *)gPokemonSets[species].ivs, (u16 *)gPokemonSets[species].moves, FALSE, FALSE, (u8)gPokemonSets[species].teraType, 0, otIdType, 0);
}

void ScrCmd_givemonset(struct ScriptContext *ctx)
{
    u16 species = VarGet(ScriptReadHalfword(ctx));
    u16 otIdType = ScriptReadByte(ctx);

    gSpecialVar_Result = GiveMonSet(species, otIdType);
}
...
void ScrCmd_createmon(struct ScriptContext *ctx)
{
    ...
    gSpecialVar_Result = ScriptGiveMonParameterized(side, slot, species, level, item, ball, nature, abilityNum, gender, evs, ivs, moves, isShiny, gmaxFactor, teraType, dmaxLevel, OT_ID_PLAYER_ID, 0);
}
