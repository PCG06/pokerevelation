...
static const u8 sSelector2[]       = INCBIN_U8("graphics/ui_battle_menu/selector_2.4bpp");

//Palettes
static const u16 sMenuPalette[]        = INCBIN_U16("graphics/ui_battle_menu/palette.gbapal");
static const u16 sMenuPalette_Blue[]   = INCBIN_U16("graphics/ui_battle_menu/palette_blue.gbapal");
static const u16 sMenuPalette_Yellow[] = INCBIN_U16("graphics/ui_battle_menu/palette_yellow.gbapal");
static const u16 sMenuPalette_Green[]  = INCBIN_U16("graphics/ui_battle_menu/palette_green.gbapal");
...
void UI_Battle_Menu_Init(MainCallback callback)
{
    ...
    for (i = 0; i < NUM_SIDE_INFO; i++)
    {
        ...
        case SIDE_INFO_SPIKES:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_PLAYER), HAZARDS_SPIKES))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_TOXIC_SPIKES:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_PLAYER), HAZARDS_TOXIC_SPIKES))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STEALTH_ROCK:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_PLAYER), HAZARDS_STEALTH_ROCK))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STEEL_SURGE:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_PLAYER), HAZARDS_STEELSURGE))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STICKY_WEB:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_PLAYER), HAZARDS_STICKY_WEB))
                isExtraInfoShown = TRUE;
            break;
    ...
    for (i = 0; i < NUM_SIDE_INFO; i++)
    {
        ...
        case SIDE_INFO_TAILWIND:
            ...
            break;
        case SIDE_INFO_SPIKES:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_OPPONENT), HAZARDS_SPIKES))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_TOXIC_SPIKES:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_OPPONENT), HAZARDS_TOXIC_SPIKES))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STEALTH_ROCK:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_OPPONENT), HAZARDS_STEALTH_ROCK))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STEEL_SURGE:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_OPPONENT), HAZARDS_STEELSURGE))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_STICKY_WEB:
            if (IsHazardOnSide(GetBattlerSide(B_SIDE_OPPONENT), HAZARDS_STICKY_WEB))
                isExtraInfoShown = TRUE;
            break;
        case SIDE_INFO_SAFEGUARD:
    ...
    for (i = 0; i < NUM_STATUS_INFO; i++)
    {
        for (j = 0; j < MAX_BATTLERS_COUNT; j++)
        {
            ...
            case STATUS_INFO_CONFUSION:
                if (gBattleMons[j].volatiles.confusionTurns > 0)
                    isExtraInfoShown = TRUE;
                break;
            ...
            case STATUS_INFO_UPROAR:
                if (gBattleMons[j].volatiles.uproarTurns > 0)
                    isExtraInfoShown = TRUE;
                break;
            case STATUS_INFO_BIDE:
                if (gBattleMons[j].volatiles.bideTurns > 0)
                    isExtraInfoShown = TRUE;
                break;
            ...
            case STATUS_INFO_DEFENSE_CURL:
                if (gBattleMons[j].volatiles.defenseCurl)
                    isExtraInfoShown = TRUE;
                break;
            ...
            case STATUS_INFO_SEMI_INVULNERABLE:
                if (gBattleMons[j].volatiles.semiInvulnerable && gBattleMons[j].volatiles.semiInvulnerable != STATE_COMMANDER)
                    isExtraInfoShown = TRUE;
                break;
            case STATUS_INFO_ELECTRIFIED:
                if (gBattleMons[j].volatiles.electrified)
                    isExtraInfoShown = TRUE;
                break;
...
static void PrintMoveInfo(u16 move, u8 x, u8 y, u8 moveIdx)
{
    ...
    u8 UNUSED moveType = GetMoveType(move);
...
static void PrintStatusTab(void)
{
    ...
    for (i = 0; i < maxLines; i++)
    {
        ...
        case STATUS_INFO_UPROAR:
            ...
            StringCopy(gStringVar1, sText_Title_Field_Turns_Left);
            AddTextPrinterParameterized4(windowId, FONT_SMALL_NARROW, (x * 8) + x2 + (SPACE_BETWEEN_LINES_FIELD * 2), (y * 8) + y2, 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar1);
            turnsLeft = gBattleMons[sMenuDataPtr->battlerId].volatiles.uproarTurns;
            ConvertIntToDecimalStringN(gStringVar1, turnsLeft, STR_CONV_MODE_LEFT_ALIGN, 4);
            AddTextPrinterParameterized4(windowId, FONT_SMALL_NARROW, (x * 8) + x2 + (SPACE_BETWEEN_LINES_FIELD * 3), (y * 8) + y2, 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar1);*/
            ...
            case STATUS_INFO_BIDE:
                ...
                StringCopy(gStringVar1, sText_Title_Field_Turns_Left);
                AddTextPrinterParameterized4(windowId, FONT_SMALL_NARROW, (x * 8) + x2 + (SPACE_BETWEEN_LINES_FIELD * 2), (y * 8) + y2, 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar1);
                turnsLeft = gBattleMons[sMenuDataPtr->battlerId].volatiles.bideTurns;
                ConvertIntToDecimalStringN(gStringVar1, turnsLeft, STR_CONV_MODE_LEFT_ALIGN, 4);
                AddTextPrinterParameterized4(windowId, FONT_SMALL_NARROW, (x * 8) + x2 + (SPACE_BETWEEN_LINES_FIELD * 3), (y * 8) + y2, 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar1);
            ...
            case STATUS_INFO_INFUATION:
            {
                ...
                {
                    if ((gBattleMons[sMenuDataPtr->battlerId].volatiles.infatuation) && 
                        (gBattleMons[sMenuDataPtr->battlerId].volatiles.infatuation == INFATUATED_WITH(i)) &&
            ...
            break;
            case STATUS_INFO_LEECHSEED:{
                u8 seedUser = gBattleMons[sMenuDataPtr->battlerId].volatiles.leechSeed;
