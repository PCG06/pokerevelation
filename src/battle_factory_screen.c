...
static void CreateFrontierFactorySelectableMons(u8 firstMonId)
{
    ...
    u8 UNUSED challengeNum = gSaveBlock2Ptr->frontier.factoryWinStreaks[battleMode][lvlMode] / 7;
    u8 UNUSED rentalRank = 0;
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        u16 monId = gSaveBlock2Ptr->frontier.rentalMons[i].monId; // Should this be changed to something else now that we load them from struct?
