...
#if BW_SUMMARY_DECAP == TRUE
static const u32 sRelearnPrompt_Gfx[]                       = INCBIN_U32("graphics/summary_screen/bw/learn_moves_prompt_decap.4bpp.smol");
#else
static const u32 sRelearnPrompt_Gfx[]                       = INCBIN_U32("graphics/summary_screen/bw/learn_moves_prompt.4bpp.smol");
#endif
...
static bool8 ExtractMonDataToSummaryStruct(struct Pokemon *mon)
{
    ...
    default:
        ...
        sMonSummaryScreen->relearnableMovesNum = P_SUMMARY_SCREEN_MOVE_RELEARNER ? GetNumberOfRelearnerMoves(mon) : 0;
        return TRUE;
    }
    sMonSummaryScreen->switchCounter++;
    return FALSE;
...
static void DrawExperienceProgressBar(struct Pokemon *unused)
{
    ...
    if (summary->level < MAX_LEVEL && !FlagGet(FLAG_FRONTIER_LEVEL_50))
    {
...
static void PrintExpPointsNextLevel(void)
{
    ...
    if (sum->level < MAX_LEVEL && !FlagGet(FLAG_FRONTIER_LEVEL_50))
...
static void SetNewMoveTypeIcon(void)
{
    ...
    else
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            ...
            SetTypeSpritePosAndPal(NUMBER_OF_MON_TYPES + GetMoveContestCategory(sMonSummaryScreen->newMove), 8, 128, SPRITE_ARR_ID_TYPE + 4);
    }
}
...
static void ShowMoveRelearner(void)
{
    if (sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_RELEARN_PROMPT] == SPRITE_NONE)
        sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_RELEARN_PROMPT] = CreateSprite(&sSpriteTemplate_RelearnPrompt, 61, 152, 0);
